...
│ Sinyal     : {sig[:28]:<28}│
│ Detail     : {sinyal[:28]:<28}│
│ Sniper     : {str(row.get("bsjp_sniper", "-"))[:28]:<28}│
...
def build_row(symbol: str, daily_df: pd.DataFrame, intraday_5m: pd.DataFrame):
    ...
    return {
        ...
        "base_signal": sinyal_base,
        "bsjp_sniper": bsjp_sniper["label"],
        "bsjp_sniper_score": bsjp_sniper["score"],
        "bsjp_reason": bsjp_sniper["reason"],
        "rvol": rvol,
        "entry": entry,
        "entry_low": entry_low,
        "entry_high": entry_high,
...
def make_html_table(df: pd.DataFrame, title: str, sub: str):
    ...
    for i, (_, row) in enumerate(df.iterrows(), start=1):
        ...
        <tr>
            ...
            <td style="background:{bg_signal(row.get('bsjp_sniper', row['sinyal']))};color:#fff;">{row.get('bsjp_sniper', '-')}</td>
            <td style="background:{bg_score(row.get('bsjp_sniper_score', 0))};color:#fff;">{int(row.get('bsjp_sniper_score', 0))}</td>
            ...
            <td style="background:#0f172a;color:#fff;">{str(row.get('bsjp_reason', '-'))[:45]}</td>
        </tr>
        """
...
if display_df.empty:
    ...
    rank_df = display_df[[
        "symbol", "emoji_signal", "bsjp_sniper", "bsjp_sniper_score", "bsjp_reason", "now", "entry_zone", "entry_status", "tp_zone", "sl",
    ...
    rank_df.columns = [
        "EMITEN", "SIGNAL", "BSJP SNIPER", "SNIPER SCORE", "SNIPER NOTE", "PRICE", "ENTRY ZONE", "ENTRY STATUS", "TP1/TP2/TP3", "SL",
...
if not display_df.empty:
    ...
    st.info(
        f"BSJP Sniper: {selected_row.get('bsjp_sniper', '-')} ({selected_row.get('bsjp_sniper_score', 0)}) | "
        f"Catatan: {selected_row.get('bsjp_reason', '-')} | "
